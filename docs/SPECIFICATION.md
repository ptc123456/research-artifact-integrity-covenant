...
- `UNRESOLVED`: otherwise, any unresolved identity/access/version field.
...
5. Contract validation rejects unknown keys, wrong count/order/source identity, unsupported values, and malformed output.
...
Any rejection, terminal non-finalized state, execution error, validator disagreement, malformed receipt, or readback mismatch enters `ERROR`. A submitted hash is persisted. Before another write, the frontend reconciles that exact transaction instead of blindly retrying.
